...
export default function ProjectCards() {
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {myProject.map((item) => {
                return (
                    <div className="group hover:scale-[1.02] transition duration-300 ease-in-out" key={item.id}>
                        <Link href={item.websiteUrl} target="_blank" rel="noopener noreferrer" className="block">
                            <div className="rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 mb-4">
                                <Image
                                    ...
                                    width={1280}
                                    height={1024}
                                    className="w-full h-48 md:h-72 object-cover group-hover:scale-105 transition-all duration-300 brightness-70 hover:brightness-90"></Image>
                            </div>
                        </Link>
                                ...
                                <div className="grow">
                                    ...
                                    <div className="py-1 text-sm max-w-sm overflow-hidden  mr-3 text-gray-500 flex justify-between">
                                        {item.tags.map((tag) => {
                                            return (
                                                <div
                                                    className="max-w-fit mr-2 rounded-sm border border-white/10 bg-gray-400/20 text-center p-1 text-ellipsis line-clamp-1"
                                                    key={tag}>
                                                    {tag}
                                                </div>
                                            );
